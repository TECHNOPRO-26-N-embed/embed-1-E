...
|:--|:--|
| 作品タイトル | シンプルアラーム |
| 一言説明（何ができるガジェットか） | 設定時刻にアラーム音を鳴らす時計 |
| 使用する共通部品（グループ共通） | アクティブブザー |
| 追加部品（個人・主なもの） | ボタン（タクトスイッチ）、RTCモジュール（DS3231）、7セグメント表示器 |
| 必須機能の数（3-1の件数） | 件 |
...
| グループ内で統一すべき仕様 | （例：共通部品のピン番号・閾値の単位）秒 |
...
[電源ON / 初期化]
  ↓（時刻・アラーム設定完了）
[待機中] ──（現在時刻＝アラーム時刻）──→ [アラーム鳴動中] ──（ボタン押下）──→ [待機中]
...
|  |  |  |
|  |  |  |
| 初期化 | RTCや表示器などの動作確認と時刻・アラーム設定を行う | 出力なし（準備のみ） |
| 待機中 | 現在時刻を監視し、アラーム時刻になるまで待つ | 7セグメント表示器に時刻を表示 |
| アラーム鳴動中 | アラーム時刻になるとブザーを鳴らし、ボタンが押されるのを待つ | ブザーが鳴動、時刻は表示 |
