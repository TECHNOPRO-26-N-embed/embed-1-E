...
- 計測結果次第で成功・失敗音が鳴るようにした。また、4桁の7セグメントディスプレイの仕様上、特定の時間を超えたら異なる結果を表示する。
...
**動いたもの：**
- 計測機能及び結果表示
- 結果に応じてパッシブブザーで異なる音を鳴らす

**動かなかった・間に合わなかったもの：**
- 動かなかったもの：結果表示後、開始ボタンを押すまで4桁の7セグメントディスプレイに結果を表示したままにすること
- 間に合わなかったもの：タイムアウト機能

- なぜ動かなかったか（わかる範囲で）：一回限りのループ内でしか回していなく、一度表示するとその時点でループを抜けてしまう。
　<!--ダイナミック点灯という方式で今回作成したが、その性質上消えてしまうから。-->
...
- 今回は追加しなかったが、ダミーの情報も混ぜることによって、より反応速度を確実に測定することが出来そう。
