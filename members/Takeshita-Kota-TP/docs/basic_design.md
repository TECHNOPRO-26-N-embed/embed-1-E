...
[電源ON / 初期化]
  ↓（初期化完了）
[待機中] ──（開始ボタンを押す）──→ [待機中] ──（ブザーとLEDが鳴る）──（計測ボタンを押す）──→ [計測] ──（計測値異常）──→ [警告]──────────┐
　 ↑　　　　　　　　　　　　                　　　　　　　　　　　　　　　　　　　　　　　　　　　　　└─（計測値正常）──→ [結果表示]─────┤
   │                                         　　 　 　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　│
   │                                         　　 　　 　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　│
   └────────────────────（結果表示後リセット）───────────────────────────────────────────────────────────────────────────────────┘
...
| 警告 | ボタン②を押した時間が一定時間以上の時にリセット | 全て動作していない状態 |
...
| success_fail | 10.000秒以内に押したかどうかの判定 | int | 2B |  |  |
| display_time | 4桁の7セグメントディスプレイに表示する秒数 | unsgined char[4] | 4B |  |  |
| buzzer_state | 成功か失敗を示すブザーの状態 | int | 2B |  |  |
...
| F01 | 必須機能① |  | LEDが光る |  |  | loop()内 |
| F02 | 必須機能② |  | パッシブブザーが鳴る |  |  | loop()内 |
| F03 | 必須機能③ |  | アクティブブザーが鳴る |  |  | loop()内 |
| F04 | 必須機能④ |  | 4桁の7セグメントディスプレイに結果表示 |  |  | loop()内 |
| F05 | 必須機能⑤ |  |  |  |  | loop()内 |
...
|:--|:--|:--|:--|
| LED点滅 | ms | delay() | ブザーの動作を同時に行う為 |
| アクティブブザーが鳴る | ms | delay() | LED点滅を同時に行う為 |
...
| LED(赤) |  |  | 出力 |220Ω抵抗と直列  |
| ボタン |  |  |  | 入力 |
| アクティブブザー |  |  | 出力 | 220Ω抵抗と直列 |
| パッシブブザー |  |  | 出力 | 220Ω抵抗と直列 |
| 4桁の7セグメントディスプレイ |  |  | 出力 | 220Ω抵抗と直列 |
