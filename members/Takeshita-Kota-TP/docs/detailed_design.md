...
【センサー・入力値】（basic_design.md 2-1 から転記）
  ...
  passive_state : bool = false  // true:鳴動 false:停止
  display_time : float = 0.000
  debounceDelay　: const unsgined long = 0
...
【処理の流れ】
1. 各種ピンモードを設定する。
  - PIN_BUTTON_1, PIN_BUTTON_2, PIN_BUZZER_ACTIVE, PIN_BUZZER_PASSIVE → INPUT_PULLUP
  - PIN_LED → OUTPUT
...
```
【処理の流れ】
1. ボタンが押されたかどうかを取得し、オン(HIGH)のままにする
2. millisでチャタリング対策
3. LOWに初期化
...
**引数：** `int num_st, int num_end `

**戻り値：** int型 number

```
【処理の流れ】
1. random(num_st, num_end)でnum_st以上num_end以下の適当な値を生成し、int numberに格納
...
4. millis()で100ms待ち、falseにして動作を止める(LEDとアクティブブザーを動作させて、ms策定予定)
