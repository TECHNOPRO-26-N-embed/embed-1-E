...
|:--|:--|:--|
| 待機中 | タイマーでリマインド時間を計測 | LED消灯 |
| リマインド中 | LEDで水分補給を促す | LED点滅 |
| 警告中 | 30秒間反応がなかった場合警告。ボタン押下で警告解除・待機に戻る | 開始時にブザー2回鳴動、その後は一定間隔で再鳴動。LED点滅 |
...
| remindLedPin | LEDの接続ピン番号 | const int | 2B | 9 | 例：D9 |
...
| F02 | 必須：LEDでリマインド状態を表示 | `updateRemindLed()` | リマインド中のLED点滅パターンを制御する | int state | なし | loop()内 |
| F03 | 必須：30秒無反応でブザー警告 | `checkWarningTimeout()` | リマインド開始から30秒経過時に警告状態へ遷移し、開始時2回鳴動＋一定間隔で再鳴動を制御する | なし | なし | loop()内 |
| F04 | 必須：ボタンでリセット | `resetByButton()` | ボタン押下時に警告解除・タイマー初期化・待機状態へ戻し、1秒以内に反映する | bool pressed | なし | loop()内 |
...
|:--|:--|:--|:--|
| LED点滅（リマインド中） | 500ms | **millis** | ボタン監視と並行しながら点滅を維持するため |
