...
| 共通部品が担う役割 | 休憩の開始(ボタンの入力と同時に1秒)、時間が経つと終了を音で知らせる(ボタンを押すと止まる) |
| 接続するArduinoピン（予定） | D12 |
| 個人の設計と共通部品が協調する箇所 | ボタン操作で休憩開始・終了を判定し、ブザーで通知。休憩終了時はシリアルモニタにも時刻を記録。 |
...
[電源ON / 初期化]
  ↓（RTCモジュールの初期化完了）
[待機中] ──（ボタン押下・開始時刻を記録）──→ [休憩中] ──（休憩時間経過・終了時刻を記録）──→ [通知中]
   ↑                                                                                      │
   └────────────────────────────（再度ボタン押下でリセット）────────────────────────────┘
```
**各状態の意味：**
...
| 休憩中   | 休憩タイマーをカウント。経過したら終了時刻を記録 | ブザーOFF、シリアル出力なし |
| 通知中   | 休憩終了をブザーで通知。ボタン押下でリセット | ブザーON（終了通知）、シリアルに開始・終了時刻を出力 |
