...
|:---|:---|:---|:---|:---|:---|
| 1 | ブザーの1秒非同期制御 | setBuzzer(true) を呼び出し、1秒後に自動停止を確認 | ブザーが1秒後に停止する | | [ ] |
| 2 | ボタン押下とブザー制御の並行動作 | ブザー鳴動中にボタンを押す | ボタン押下イベントが正しく検出される | | [ ] |
| 3 | millis()タイマーの精度 | 休憩時間を計測し、ストップウォッチで確認 | 設定した休憩時間通りに動作する | | [ ] |
